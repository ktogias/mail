OC.L10N.register(
    ...
    "Mail server marked this message as phishing attempt" : "メールサーバーはこのメッセージをフィッシング攻撃としてマークしました",
    "Some addresses in this message are not matching the link text" : "このメッセージの一部のアドレスがリンクテキストと一致しません",
    "Reply-To email: %1$s is different from sender email: %2$s" : "返信先の電子メール: %1$s は送信者の電子メール: %2$s と異なります",
    "Mail connection performance" : "メール接続パフォーマンス",
    "Slow mail service detected (%1$s) an attempt to connect to several accounts took an average of %2$s seconds per account" : "メールサービスの遅延が検出されました（%1$s）。複数のアカウントへの接続試行に、アカウントあたり平均%2$s秒かかりました。",
    "Slow mail service detected (%1$s) an attempt to perform a mail box list operation on several accounts took an average of %2$s seconds per account" : "メールサービスの遅延が検出されました（%1$s）複数のアカウントでメールボックスリスト操作を実行しようとしたところ、アカウントあたり平均%2$s秒かかりました",
    ...
    "IMAP server denied authentication" : "IMAPサーバーが認証を拒否しました",
    "SMTP server denied authentication" : "SMTPサーバーが認証を拒否しました",
    "IMAP authentication error" : "IMAP認証エラー",
    "SMTP authentication error" : "SMTP認証エラー",
    ...
    "Password" : "パスワード",
    "Enable mark as important classification" : "重要分類としてマーク",
    "Manual" : "手動",
    ...
    "Mail settings" : "メール設定",
    "General" : "一般",
    "Set as default mail app" : "既定のメールアプリとして設定",
    ...
    "This setting only makes most sense if you use the same user back-end for your Nextcloud and mail server of your organization." : "この設定は、組織のNextcloudとメールサーバーに同じユーザーバックエンドを使用している場合にのみ意味があります。",
    ...
    "Tenant ID (optional)" : "テナントID（オプション）",

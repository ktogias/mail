...
describe('Envelope', () => {
	...
	describe('preview text', () => {
		// Truncated in JS instead of relying on -webkit-line-clamp, which
		// is known to cache its "how many lines fit" calculation and fail
		// to recompute after certain incremental DOM updates -- confirmed
		// live as envelope rows permanently overlapping, surviving both a
		// CSS containment fix and a hard refresh.
		function mountEnvelope(dataOverrides = {}) {
			return shallowMount(Envelope, {
				mocks: { $route },
				propsData: {
					mailbox: {
		...
		it('leaves short preview text untouched', () => {
			const view = mountEnvelope({ previewText: 'A short preview.' })

			expect(view.vm.previewTextForSubtitle).toBe('A short preview.')
		})

		it('truncates long preview text at a word boundary with an ellipsis', () => {
			const long = 'word'.repeat(60) + ' ' + 'tail'
			const view = mountEnvelope({ previewText: long })

			const result = view.vm.previewTextForSubtitle
			expect(result.length).toBeLessThan(long.length)
			expect(result.endsWith('…')).toBe(true)
			expect(result).not.toContain('tail')
		})

		it('prefers the AI summary over the raw preview text when both are present', () => {
			const view = mountEnvelope({ previewText: 'raw preview', summary: 'AI summary' })

			expect(view.vm.previewTextForSubtitle).toBe('AI summary')
		})

		it('shows a fixed label for encrypted messages instead of any preview content', () => {
			const view = mountEnvelope({ encrypted: true, previewText: 'this must never be shown' })

			expect(view.vm.previewTextForSubtitle).toBe('Encrypted message')
		})
	})
})

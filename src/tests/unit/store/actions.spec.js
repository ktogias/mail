...
describe('Vuex store actions', () => {
	...
	describe('progressive newest-first text search', () => {
		...
		/**
		 * A `body:` term makes a search request a live IMAP SEARCH, and a
		 * unified search issues one PER MAILBOX -- measured live at 2.7-6 s
		 * each, concurrency 2, first useful response 77 seconds in. The header
		 * half of the same search is a database query the server answers in
		 * ~100 ms, so it must not be held behind the other.
		 */
		it('asks for headers before bodies in a unified search', async () => {
			normalizedEnvelopeListId.mockImplementation((query) => query ?? '')
			const account = { id: 13 }
			store.addAccountMutation(account)
			store.addMailboxMutation({
			...
			await store.fetchSharedUnifiedContentSearch({
				mailbox: store.mailboxes[UNIFIED_INBOX_ID],
				query: 'subject:needle body:needle',
				descriptor: { baseQuery: 'subject:needle body:needle' },
				searchUpperBound: 1_900_000_000,
			})

			const queries = () => MessageService.fetchEnvelopes.mock.calls.map((call) => call[2])
			// Whatever the first pass asked for, it did not reach IMAP.
			expect(queries()[0]).not.toContain('body:')
			expect(queries()[0]).toContain('subject:needle')
			// ...and the body pass follows on its own, without having been
			// awaited by anything the user is waiting for.
			await vi.waitFor(() => {
				expect(queries().some((query) => query.includes('body:needle'))).toBe(true)
			})
		})

		/**
		 * A filter whose ONLY text predicate is `body:` strips down to its
		 * structural tokens, and those match every message rather than none.
		 * Splitting it would flood the list with unrelated mail.
		 */
		it('does not split a filter that has nothing but body terms', async () => {
			normalizedEnvelopeListId.mockImplementation((query) => query ?? '')
			const account = { id: 13 }
			store.addAccountMutation(account)
			store.addMailboxMutation({
			...
			await store.fetchSharedUnifiedContentSearch({
				mailbox: store.mailboxes[UNIFIED_INBOX_ID],
				query: 'body:needle',
				descriptor: { baseQuery: 'body:needle' },
				searchUpperBound: 1_900_000_000,
			})

			MessageService.fetchEnvelopes.mock.calls.forEach((call) => {
				expect(call[2]).toContain('body:needle')
			})
		})
	...
	describe('backfill banner dismissal (dismissBackfillBannerMutation)', () => {
		it('records a session dismissal for a mailbox', () => {
